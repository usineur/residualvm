...
SwitchSdlEventSource::SwitchSdlEventSource() {
...
bool SwitchSdlEventSource::handleControllerButton(const SDL_Event &ev, Common::Event &event, bool buttonUp) {
	...
	if (shouldGenerateMouseEvents()) {
		...
		bool isEMI = Grim::g_grim->getGameType() == Grim::GType_MONKEY4;

		static const ControllerEventMapping mapping[] = {
			// B : Examine item
			{ EVENT_KEYDOWN, KeyState(KEYCODE_e), EVENT_INVALID, KeyState() },
			// A : Use item / Talk to character (GRIM) - Default action (EMI)
			{ EVENT_KEYDOWN, KeyState(KEYCODE_RETURN), EVENT_INVALID, KeyState() },
			// Y : Pick up item / Put away item
			{ EVENT_KEYDOWN, KeyState(KEYCODE_p), EVENT_INVALID, KeyState() },
			// X : Open inventory
			{ EVENT_KEYDOWN, KeyState(KEYCODE_i), EVENT_INVALID, KeyState() },
			// - : Virtual keyboard
			{ EVENT_VIRTUAL_KEYBOARD, KeyState(), EVENT_INVALID, KeyState() },
			{ EVENT_INVALID, KeyState(), EVENT_INVALID, KeyState() },
			// + : Open help
			{ EVENT_KEYDOWN, KeyState(KEYCODE_F1), EVENT_INVALID, KeyState() },
			{ EVENT_INVALID, KeyState(), EVENT_INVALID, KeyState() },
			{ EVENT_INVALID, KeyState(), EVENT_INVALID, KeyState() },
			// L : Skip cutscene
			{ EVENT_KEYDOWN, KeyState(KEYCODE_ESCAPE), EVENT_INVALID, KeyState() },
			// R : Run
			{ EVENT_KEYDOWN, KeyState(KEYCODE_LSHIFT), EVENT_INVALID, KeyState() },
			// Move up (GRIM) - Cycle objects up (EMI)
			{ EVENT_KEYDOWN, KeyState(isEMI ? KEYCODE_PAGEUP : KEYCODE_UP), EVENT_INVALID, KeyState() },
			// Move down (GRIM) - Cycle objets down (EMI)
			{ EVENT_KEYDOWN, KeyState(isEMI ? KEYCODE_PAGEDOWN : KEYCODE_DOWN), EVENT_INVALID, KeyState() },
			// Move left
			{ EVENT_KEYDOWN, KeyState(KEYCODE_LEFT), EVENT_INVALID, KeyState() },
			// Move right
			{ EVENT_KEYDOWN, KeyState(KEYCODE_RIGHT), EVENT_INVALID, KeyState() }
		};
...
bool SwitchSdlEventSource::handleControllerAxisMotion(const SDL_Event &ev, Common::Event &event) {
	if (shouldGenerateMouseEvents()) {
		return ResVmSdlEventSource::handleControllerAxisMotion(ev, event);
	} else {
		bool isEMI = Grim::g_grim->getGameType() == Grim::GType_MONKEY4;
		...
		if (ev.caxis.axis == SDL_CONTROLLER_AXIS_TRIGGERLEFT) {
			...
			if (l2Pushed == pushed)
				return false;
			event.type = (pushed) ? Common::EVENT_KEYUP : Common::EVENT_KEYDOWN;
			// Skip dialogue line
			event.kbd.keycode = isEMI ? Common::KEYCODE_DELETE : Common::KEYCODE_KP_PERIOD;
			l2Pushed = pushed;
			return true;
			...
			event.type = (pushed) ? Common::EVENT_KEYUP : Common::EVENT_KEYDOWN;
			// Use / Talk (EMI only)
			event.kbd.keycode = isEMI ? Common::KEYCODE_u : Common::KEYCODE_INVALID;
			r2Pushed = pushed;
			return true;
		} else {
...
void SwitchSdlEventSource::preprocessEvents(SDL_Event *event) {
...
void SwitchSdlEventSource::preprocessFingerDown(SDL_Event *event) {
	// front (0) or back (1) panel
	SDL_TouchID port = event->tfinger.touchId;
	// id (for multitouch)
...
void SwitchSdlEventSource::preprocessFingerUp(SDL_Event *event) {
	// front (0) or back (1) panel
	SDL_TouchID port = event->tfinger.touchId;
	// id (for multitouch)
	SDL_FingerID id = event->tfinger.fingerId;
...
void SwitchSdlEventSource::preprocessFingerMotion(SDL_Event *event) {
	// front (0) or back (1) panel
	SDL_TouchID port = event->tfinger.touchId;
	// id (for multitouch)
	SDL_FingerID id = event->tfinger.fingerId;
...
void SwitchSdlEventSource::convertTouchXYToGameXY(float touchX, float touchY, int *gameX, int *gameY) {
...
void SwitchSdlEventSource::finishSimulatedMouseClicks() {
	for (int port = 0; port < SCE_TOUCH_PORT_MAX_NUM; port++) {
		for (int i = 0; i < 2; i++) {
